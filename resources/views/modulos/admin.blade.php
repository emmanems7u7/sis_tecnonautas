...
@section('content')
    @if(Auth::user()->hasRole('admin') || Auth::user()->hasRole('profesor') || Auth::user()->hasRole('Demo'))
        ...
        <div class="card mt-3">
            <div class="card-body">
                ...
                <div class="table-responsive">
                    ...
                    <table id="tabla-estudiantes" class="table-sm table table-striped" style="display: none;">
            ...
            function notasEstudiantes() {
                ...
                fetch("{{ route('notasEstudiantes.ver', ['id_a' => $id_a, 'id_m' => $id_m, 'id_p' => $id_p]) }}")
                    ...
                    .then(data => {
                        ...
                        p.colSpan = allEvaluacionesIds.size;
                        p.style.textAlign = 'center';

                        // Crear enlace para exportar en PDF
                        @can('notas.exportar_pdf')
                            const enlacePDF = document.createElement('a');
                            enlacePDF.href = "{{ route('notas.exportar_pdf', [$id_a, $id_m, $id_p]) }}";
                            enlacePDF.target = "_blank";
                            enlacePDF.textContent = 'Exportar PDF';
                            enlacePDF.classList.add('btn', 'btn-danger', 'btn-sm', 'mx-1');
                            p.appendChild(enlacePDF);
                        @endcan
                            @can('notas.exportar_excel')

                                const enlaceExcel = document.createElement('a');
                                enlaceExcel.href = "{{ route('notas.exportar_excel', [$id_a, $id_m, $id_p]) }}";
                                enlaceExcel.target = "_blank";
                                enlaceExcel.textContent = 'Exportar Excel';
                                enlaceExcel.classList.add('btn', 'btn-success', 'btn-sm', 'mx-1');
                                p.appendChild(enlaceExcel);
                            @endcan
                        ...
                        estudiantesArray.forEach(item => {
                            ...
                            celdaAcciones.innerHTML = `
                                                                                                                                                                                                        <a href="${'{{ route('estudiante.detalle', ['id' => ':id', 'id_p' => $id_p, 'id_a' => $id_a, 'id_m' => $id_m]) }}'.replace(':id', item.user_id)}" class="btn btn-sm btn-primary mb-1" target="_blank" >Detalles</a>
            ...
            function datosPersonal(idEstudiante) {
                ...
                fetch(dat)
                    ...
                    .then(data => {
                        ...
                        const evaluaciones_est = data.evaluacionesEstudiante;
                        ...
                        evaluaciones_est.forEach(evaluacion => {
                            const row = `
                                                                                                                                                                                <tr>
